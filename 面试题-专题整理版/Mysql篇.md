...
首先，三范式（3NF）也叫数据库规范化（Normalization）。
...
索引的底层数据结构是B+树。B+树的特点是非叶子节点不存储具体数据，只存储索引数据和指针。叶子节点存储索引数据和整行数据；并且叶子节点有双向链表，优化范围查找；非叶子节点有冗余数据
...
3. 联合索引（复合索引）：联合多个字段进行索引。
4. 全文索引：用于大文本字段（如 `CHAR`、`VARCHAR`、`TEXT`），支持分词搜索
5. 空间索引：用于地理空间数据类型（如 `GEOMETRY`、`POINT`），只有 `MyISAM` 和 `InnoDB` 支持。
...
回表是通过二级索引找到组件值，再通过主键值回到主键索引查询数据。 
...
有，定位那些有慢sql，查看执行计划（Execution Plan）
